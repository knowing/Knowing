...
object OSGIUtil {
  ...
  def createServiceProperties(factory: TFactory): Dictionary[String, _] = {
    ...
    properties.put(TFactory.FACTORY_ID, factory.id)
    properties.put(TFactory.FACTORY_NAME, factory.name)
    //TODO properties.put(TFactory.PROCESSOR_CLASS, factory.getClass.getName)
    properties
  }
}
